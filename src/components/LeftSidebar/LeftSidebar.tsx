    ...
    <aside className="w-full lg:w-1/4 space-y-6">
      <div className="bg-white rounded-lg shadow p-6 space-y-6">
        <div className="flex items-center space-x-4">
          <Image src="/placeholder.svg?height=64&width=64" alt="User Avatar" width={64} height={64} className="w-16 h-16 rounded-full" />
          <div>
        ...
        <nav className="space-y-2">
          {menuItems.map((item) => (
            <Link key={item.label} href={item.href} className="flex items-center space-x-3 px-4 py-2 text-gray-700 rounded-lg hover:bg-gray-100">
